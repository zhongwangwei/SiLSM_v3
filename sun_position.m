function [zenith,azimuth] = sun_position(Year,Month,Day,Hour,Minute, location)
        time.year = Year;        
        time.month = Month;
		time.day = Day;  
		time.hour = Hour;
...
[zenith,azimuth] = sun_topocentric_zenith_angle_calculate(location, topocentric_sun_position, topocentric_local_hour);
...
function [zenith,azimuth] = sun_topocentric_zenith_angle_calculate(location, topocentric_sun_position, topocentric_local_hour)
...
azimuth = (atan2(nominator, denominator) * 180/pi) + 180;
% Set the range to [0-360]
azimuth = set_to_range(azimuth, 0, 360);
